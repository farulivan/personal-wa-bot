...
- [x] ~~Guard the profile at startup~~ — **obsolete.** The bot moved to Baileys in August 2026 ([ADR 0005](../adr/0005-whatsapp-transport.md)), so there is no browser, no pinned Chrome and no profile on the volume. The whole failure mode is gone rather than guarded. [#56](https://github.com/farulivan/personal-wa-bot/issues/56) closed unimplemented.
- [x] Delete `session.bak` and the live `session` profile from the Railway volume — done 2026-08-16. Both held back deliberately until the Baileys migration ([ADR 0005](../adr/0005-whatsapp-transport.md)) merged to `main`, because the working profile was what kept a rollback free of a QR re-scan. Note the volume path: `LocalAuth` was given `RAILWAY_VOLUME_MOUNT_PATH` directly, so on Railway these were `<volume>/session` and `<volume>/session.bak` — the name `.wwebjs_auth` only ever applied to local development.
