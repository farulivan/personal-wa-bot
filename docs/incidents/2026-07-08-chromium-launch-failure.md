...
- [ ] Delete `session.bak` **and** the old `.wwebjs_auth` profile from the Railway volume. Deliberately still pending: `.wwebjs_auth` is what makes a rollback to the pre-migration deploy free, so both go once the Baileys migration merges to `main` and the rollback window closes.
